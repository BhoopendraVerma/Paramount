...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        PayPalMobile.initializeWithClientIds(forEnvironments: [PayPalEnvironmentProduction: "APP_ID_PAYPAL_LIVE_ENVIORNMENT", PayPalEnvironmentSandbox: "ankit.yugasa-facilitator@gmail.com"])
        ...
        if let wind = UIApplication.shared.delegate?.window {
            if let rootViewController = wind?.rootViewController {
                ...
                if let alreadySomeOneThere = rootViewController.presentedViewController {
                    alreadySomeOneThere.present(nav1, animated: true, completion: nil)
                }else {
                    rootViewController.present(nav1, animated: true, completion: nil)
                }
            }
        }
    }
